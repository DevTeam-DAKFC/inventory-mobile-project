...
final class PaginatedImportBatchErrorRestDto {
  const PaginatedImportBatchErrorRestDto({
    required this.items,
    required this.total,
    required this.page,
  ...
  final List<ImportBatchErrorRestDto> items;
  final int total;
  final int page;
  final int pageSize;
  final bool hasNextPage;

  factory PaginatedImportBatchErrorRestDto.fromJson(Map<String, dynamic> json) {
    final items = json['items'];
    if (items is! List) {
      throw _invalidPaginatedField('items', json);
    }

    return PaginatedImportBatchErrorRestDto(
      items: items
          .map(
            (item) => ImportBatchErrorRestDto.fromJson(
...
AppException _invalidPaginatedField(String field, Map<String, dynamic> json) {
  return AppException(
    ...
    message:
        'Invalid import batch errors page response: field "$field" is invalid.',
