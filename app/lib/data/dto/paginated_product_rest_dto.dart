...
final class PaginatedProductRestDto {
  const PaginatedProductRestDto({
    required this.items,
    required this.total,
    required this.page,
  ...
  factory PaginatedProductRestDto.fromJson(Map<String, dynamic> json) {
    final items = json['items'];
    if (items is! List) {
      throw _invalidPaginatedField('items', json);
    }

    return PaginatedProductRestDto(
      items: items
          .map(
            (item) =>
                ProductRestDto.fromJson(Map<String, dynamic>.from(item as Map)),
          )
          .toList(),
...
AppException _invalidPaginatedField(String field, Map<String, dynamic> json) {
  return AppException(
    ...
    message: 'Invalid products page response: field "$field" is invalid.',
