...
Widget _harness({required _MockAuthRepository repository, required AppSession session}) {
  return ProviderScope(
    overrides: [
      authRepositoryProvider.overrideWithValue(repository),
      appSessionProvider.overrideWithValue(session),
    ],
    child: MaterialApp.router(routerConfig: _buildAuthAwareRouter(session)),
  );
}
...
void main() {
  ...
  testWidgets('shows the "Cerrar sesión" action with a logout icon when authenticated', (
    tester,
  ) async {
  ...
  testWidgets('tapping "Cerrar sesión" calls AuthRepository.logout, signs out the '
      'AppSession and the router redirects to /login', (tester) async {
    when(() => repository.logout()).thenAnswer((_) async => const AppSuccess<void>(null));
  ...
  testWidgets('duplicate taps while logout is loading do not trigger duplicate logout '
      'calls', (tester) async {
