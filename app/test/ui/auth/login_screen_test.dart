...
void main() {
  ...
  testWidgets('renders the auth-shell sign-in copy and the primary CTA', (
    tester,
  ) async {
    ...
    expect(
      find.text('Ingresá para gestionar inventario entre sucursales.'),
      findsOneWidget,
    );
  ...
  testWidgets(
    'empty submit shows required-field errors and does not call login',
    (tester) async {
  ...
  testWidgets(
    'AppSuccess calls setAuthenticatedUser and the router lands on /home',
    (tester) async {
  ...
  testWidgets('AppFailure unauthorized shows the Spanish credentials banner', (
    tester,
  ) async {
    when(
      () => repository.login(
