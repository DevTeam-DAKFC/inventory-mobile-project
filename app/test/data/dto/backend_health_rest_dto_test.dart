...
void main() {
  group('BackendHealthRestDto', () {
    test('parses valid JSON', () {
      final dto = BackendHealthRestDto.fromJson(
        <String, dynamic>{'status': 'ok', 'service': 'Inventory.Api'},
      );
    ...
    test('throws AppException with code unexpected when status is missing',
        () {
      expect(
        () => BackendHealthRestDto.fromJson(
          <String, dynamic>{'service': 'Inventory.Api'},
        ),
        throwsA(
          isA<AppException>()
    ...
    test('throws AppException with code unexpected when service is missing',
        () {
      expect(
    ...
    test('throws AppException when status is not a string', () {
      expect(
        () => BackendHealthRestDto.fromJson(
          <String, dynamic>{'status': 1, 'service': 'Inventory.Api'},
        ),
        throwsA(
          isA<AppException>()
              .having((e) => e.code, 'code', AppErrorCode.unexpected),
    ...
    test('throws AppException when service is not a string', () {
      expect(
        () => BackendHealthRestDto.fromJson(
          <String, dynamic>{'status': 'ok', 'service': 42},
        ),
        throwsA(
          isA<AppException>()
              .having((e) => e.code, 'code', AppErrorCode.unexpected),
